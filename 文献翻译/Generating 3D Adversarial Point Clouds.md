...
说：位于临界点（Critical Points）和上界（Upper Bounds）之间的点集会产生相同的全局特征，因此 PointNet 对点丢失和随机扰动具有鲁棒性

1. 什么是全局特征（Global Feature）？

不论下面盖了多少东西，我们只关心**这块布最终被撑起来的样子**。这就是 PointNet 提取到的“全局特征”。

2. 什么是临界点（Critical Points）？

在这一堆杂物中，只有几个**最高的、最突出的物体**真正顶住了布，决定了布的形状。

- 比如：一个竖着的扫把、一把椅子的靠背。
- **这些就是临界点**。如果你把扫把抽走，布就会塌下去一块，形状（特征）就变了。

3. 什么是“介于两者之间的点集”？

现在，你可以往布下面随意**添加**或**移动**各种小东西：
...
1. **不撤走支柱（保留临界点）：** 那个扫把和椅子必须还在原位。
2. **不戳破天花板（不超过上界）：** 你新加进去的苹果、枕头、书，高度都**不能超过**原来的扫把和椅子。

**结果：** 从外面看，这块布撑起来的形状（全局特征）**完全没有变化**。



🧠 对应到 PointNet 算法：

- **Max Pooling 的逻辑**：PointNet 就像是在选“最高分”。
  - 假设一个特征向量是 `[0, 0, 0]`。
  - 点 A 贡献了特征 `[10, 2, 5]`。
  - 点 B 贡献了特征 `[3, 8, 1]`。
  - 点 C 贡献了特征 `[1, 1, 1]`。
  - **全局特征（取最大值）**：`[10, 8, 5]`。
- **临界点**：点 A 贡献了第一和第三个维度的最大值，点 B 贡献了第二个维度的最大值。它们是“支柱”。
- **非临界点**：点 C 的所有特征都比 A 和 B 小。它就是那个“被盖在帐篷下的小苹果”。
- **结论**：你可以随便修改点 C，或者加无数个像点 C 这样的点。只要它们的数值不超过 A 和 B 设定的上限，PointNet **根本“看不见”它们**，输出的全局特征永远是 `[10, 8, 5]`。

**原始 PointNet 的鲁棒性**：因为有这个特性，PointNet 能够忽略很多噪声（那些藏在帐篷下的小碎点）。

**攻击的切入点**：反过来说，如果要攻击 PointNet，**瞎改那些无关紧要的点是没用的**。必须去攻击那些**“临界点”**（把支柱打断），或者添加新的点去**突破“上界”**（制造一个比扫把还高的新支柱，改变帐篷形状）。
