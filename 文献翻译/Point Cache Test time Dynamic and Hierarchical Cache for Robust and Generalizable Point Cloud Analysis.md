...
​		本文提出了一种通用解决方案，旨在使点云识别模型能够在测试时处理 **分布偏移** (distribution shifts)。不同于以往严重依赖训练数据（通常在在线推理期间无法获取）且仅限于识别训练期间预定义的固定点云类别集的方法，我们探索了一种更实用且更具挑战性的场景：仅基于在线测试数据调整模型，以便在测试时识别先前见过的类别以及新颖的、**未见过的类别** (unseen classes)。为此，我们开发了 **Point-Cache**，这是一个`分层缓存模型`，它能够捕捉在线测试样本的关键线索，特别关注点云的全局结构及其局部部件细节。Point-Cache 作为一个丰富的 3D 知识库，通过动态管理以优先纳入高质量样本。作为一个 **即插即用** (plug-and-play) 模块，我们的方法可以灵活地集成到 **大型多模态 3D 模型**中，以支持开放词汇点云识别。值得注意的是，我们的解决方案完全无需训练，因此其运行效率与零样本推理相当。Point-Cache 在 8 个具有挑战性的基准测试和 4 个代表性的大型 3D 模型上展示了巨大的收益，突显了其有效性。代码已发布于 <https://github.com/auniquesun/Point-Cache>。

## 1 Introduction

​		近年来，**激光雷达** (LiDARs) 和 RGB-D 相机等 3D 传感器因其提供可靠 3D 几何测量的能力，已被广泛应用于机器人和电动汽车领域 [11, 16, 20, 31, 59, 61-63, 68]。点云是这些 3D 传感器产生的最直接的数据格式之一。尽管在 3D 点云识别方面已经取得了显著进展，但这些成功主要基于一个假设，即`测试数据和模型训练数据是同分布的`。然而，由于复杂的几何结构以及传感和处理误差，这一假设在现实场景中经常被违反。
...
​		在这项工作中，为了充分利用测试期间的数据分布，我们开发了一个缓存模型来存储测试样本的`关键指纹`。这些指纹由大型多模态 3D 模型派生的键值对 (key-value pairs) 组成，其中键代表由点编码器提取的样本特征，值代表模型预测的`（伪）标签`。 我们的缓存模型是分层的，因为它不仅在全局缓存中存储点云的全局特征，还在`局部缓存中存储其局部部件细节`，这对于区分类别之间的细微差异至关重要。我们还通过实证证明，这种从粗到细的设计增强了野外环境下的鲁棒性和泛化能力。此外，我们的缓存模型是动态的，因为我们需要不断用在线测试数据更新缓存，以优先保留高质量的键值对。构建的缓存作为一个强大的 3D 知识库，捕捉在线测试数据的关键特征。当新样本到达时，我们查询知识库，计算样本与缓存键（缓存的样本特征）之间的`亲和度`，并通过相应地`加权`缓存的（伪）标签来产生预测。`我们的方法是免训练且灵活的，仅需要一个预训练的 3D 模型和测试数据`。更多细节将在第 3 节中提供。
...
​		**预备知识 (Preliminary)**。我们首先回顾现有**大型多模态 3D 模型** (large multimodal 3D models) [29, 72, 73, 81, 88, 89] 的零样本推理过程。这些模型使用独立的编码器将输入的点云 $P\in\mathbb{R}^{N\times3}$ 和文本 $T$（例如，“a point cloud object of a {class}”）映射到一个共享的特征空间。记点云编码器为 $f_{p}(\cdot)$，文本编码器为 $f_{t}(\cdot)$，我们可以获得点云 $P$ 的 **全局特征** (global feature) $e_{p}^{g}=f_{p}(P)\in\mathbb{R}^{d}$ 以及文本特征 $e_{t}=f_{t}(T)\in\mathbb{R}^{d}$，其中 $d$ 是特征维度。为了**以零样本方式**识别输入点云 $P$，这些模型将文本 $T$ 中的“{class}”替换为 $C$ 个可能的类别名称中的每一个，并使用公式 (1) 计算类概率 $\hat{y}_{zs}$。
$$
...
(1) 一个直接的方法可能涉及使用额外的预训练分割网络来分割点云部件，但这在在线推理期间计算成本过高。相反，我们提出了`部件的模糊定义`。具体而言，由于大型 3D 模型点编码器的架构基于 **Transformer** [8, 57, 77] 且输入是一系列点补丁 (point patches)，我们避免根据物理语义（例如飞机的头、机身、机翼和尾部）进行严格的部件定义，而哦 `
(2) 第二个挑战是长的点补丁序列可能会占用大量资源；例如，ULIP-2 [73] 中的点编码器为每个对象生成 512 个点补丁，这将需要大量内存。为了缓解这个问题，我们建议使用 K-Means **聚类** (Clustering) 将这些点补丁总结为 $m$ 个中心 $e_{p}^{l}\in\mathbb{R}^{m\times d}$（$m<10$），与存储所有补丁相比，内存使用量减少了两个数量级。
...
**分布偏移 (Distribution Shifts)**：[测试数据与训练数据的统计特征或概率分布不一致的现象]。[例如：模型在实验室采集的理想 3D 模型数据上训练，但测试时遇到了含有严重传感器噪声或遮挡的点云数据]。



**零样本 (Zero-shot)**：[模型在部署或推理时，能够直接识别或处理在训练阶段从未遇到过的全新类别的能力]。[例如：模型只接受过“椅子”和“桌子”的点云训练，但在测试时仅凭借文本提示“具有四个轮子的交通工具”，就能成功识别出点云中的“汽车”]。



**测试时适应 (Test-time Adaptation)**：[一种在模型部署阶段，无需重新访问初始训练集，仅利用不断到来的目标域测试数据对预训练模型进行在线微调和校准的技术]。[例如：一个在白天数据上训练好的视觉系统被部署在夜间环境，它会在输出预测的同时，利用当前的夜间图像特征实时调整自身的参数以适应新环境]。



**大型多模态 3D 模型 (Large Multimodal 3D Models)**：[通过海量跨模态数据集（例如包含 3D 点云、对应 2D 渲染图以及自然语言文本描述的三元组）预训练出的大规模参数网络，擅长在不同模态之间建立深层次语义映射]。[例如：这类模型不仅能“看懂”椅子的 3D 结构，还能将其与 2D 照片中的椅子形象以及英文单词 "Chair" 的语义概念关联起来]。



**大型3D模型的零样本预测**（在公式中记为 $\hat{y}_{zs}$），指的是预训练的大型多模态3D模型（如 ULIP-2, OpenShape 等）在**不经过任何目标测试集训练或微调**的情况下，直接对当前输入的新点云样本做出的**初始分类概率** 



`1. 学习源域和目标域之间的共享特征空间是什么意思？`

**背景**：在深度学习中，“源域”通常指我们用来训练模型的干净数据集（比如实验室里生成的完美 3D 椅子），“目标域”则是模型实际应用时遇到的真实数据（比如激光雷达在街上扫出来的一把残缺的椅子）。这两者的分布是不一样的。

**具体含义**：这种方法（即领域自适应 Domain Adaptation）的核心是**“求同存异”**。在训练时，算法会强迫网络去寻找源域数据和目标域数据的共同本质属性，从而在数学高维空间中，把它们映射到同一个区域（共享特征空间） 。这样一来，无论输入的是完美的实验室椅子，还是残缺的真实椅子，模型提取出来的特征向量都差不多，从而正确分类。



`2.应用元学习框架通过训练多个分类器来处理几何偏移是什么意思？`

元学习的核心理念是学会学习，研究人员不会只训练一个固定的分类器去死记硬背数据。相反，他们会人为地制造出各种几何形变的点云（比如旋转的、缩放的、加了噪声的），然后为这些不同的形变分别训练一系列的小型分类器 。模型通过在这个过程中不断练习，掌握了“当遇到未知形变时，该如何快速调整自己”的元知识。到了真正测试时，它就能利用这些积累的“元经验”快速适应新的形变。



`3.什么是几何偏移，它和分布偏移有什么区别？`

**分布偏移 (Distribution Shifts)**：这是一个**宏观大概念（包含一切变化）**。只要测试时的数据分布和训练时不一致，都叫分布偏移。在原文中，它包括了超出分布的样本（OOD，比如遇到了训练集中根本没有的新类别）、数据损坏（Data Corruptions，如传感器故障导致的异常数据）等各种情况 。

**几何偏移 (Geometry Shifts)**：这是一个**微观子概念**，是分布偏移的一种具体表现形式。在 3D 点云领域，它专门指代物理三维空间中形状和结构的改变，比如点云被旋转了、缩放了、局部被遮挡（切掉了一块）或者产生了表面抖动（Jittering）。



`4.设计可学习的提示以从预训练的大型3D模型中引出通用知识是什么意思？`

**背景**：大型 3D 模型（如 ULIP-2）参数量极大，针对某个特定的小任务去全参数微调（Fine-tuning）它成本太高，且容易破坏它本身学到的通用知识。

**具体含义**：这被称为“提示学习 (Prompt Learning)”。研究者冻结大模型的主体参数，只是在输入的文本或点云序列前，人为加上一段可以被训练更新的特征向量（即可学习的提示） 。这就好比你给大模型下达了一个定制化的“指令”或“暗号”，这个指令在训练中不断优化，最终能最完美地把大模型内部潜藏的关于这类 3D 物体的通用常识给“激发（elicit）”出来，从而适应当前任务。



`5.什么是测试时适应策略（test-time adaptation strategies）`

指模型在部署后的测试阶段（也就是真正干活的时候），尝试根据当前遇到的新数据，对自身进行动态调整，从而改善预测性能 
