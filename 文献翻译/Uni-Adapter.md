...
`图2：方法概述。`给定一个测试点云 $X_t \in \mathbb{R}^{L \times 3}$，我们的方法通过点云编码器提取点云特征 $f_t$。3D缓存通过在线原型构建进行更新，其中聚类中心作为3D原型。原型重分配 (Prototype Reassignment) 模块细化这些原型，并计算它们与 $f_t$ 的亲和度以获得 $s^{cache}$。最后，通过使用熵驱动的置信度加权 融合 $s^{cache}$ 和模型的基础输出 $s^{main}$，获得预测逻辑值 $s^{final}$。
